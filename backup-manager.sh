...
checkbackups(){
    ...
    for ((i=0;i<=6;i++)); do data_brasil[i]=$(date -d "${datas[$i]}" +"%d/%m/%Y");

    hora_ajustada[i]=$(date -d "${horas[$i]} 3 hours ago" +"%H:%M:%S");

    if [[ "${horas[$i]}" < "18:00:00" ]]; then data_brasil[i]=$(date -d "${datas[$i]} 1 day ago" +"%d/%m/%Y"); fi;done;

    echo -e '\nBackups disponiveis para CTID: \e[32m'"$ctid"'\e[0m UUID: \e[32m'"$uuid"'\e[0m\n';

    for ((i=0;i<=6;i++)); do echo -e '\e[32m'$i - "${data_brasil[$i]}" "${hora_ajustada[$i]}"'\e[0m'; done;

    if [[ "$2" == "" ]]; then exit 0; else restorenode "$2";fi };
restorenode(){
    echo -e '\nSELECIONADA A OPÇÃO \e[32m'"$restore" - "${datas[$restore]}" "${horas[$restore]}"'\e[0m CONFIRMA? (Caso caso não aperte CTRL+C dentro de 15 segundos.)';sleep 15;
    
    echo -e '\nRestaurando para \e[91m'"${backups[$restore]}" "${datas[$restore]}" "${horas[$restore]}"'\e[0m'; echo "Restaurando para ${backups[$restore]} ${datas[$restore]}\n";
    vzctl stop "$ctid";
